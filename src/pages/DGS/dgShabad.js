...
function DgShabadLine() {
  //  const location = useLocation();
    ...
    const handleShareModal = (platform,shabad,line,punjabi,translit) => {
        ...
        switch (platform) {
            case 'facebook':
                shareLink = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl + `/shared/dasam-granth/shabad/${shabad}/line/${line}`)}&t=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'twitter':
                shareLink = `https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl + `/shared/dasam-granth/shabad/${shabad}/line/${line}`)}&text=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'youtube':
                ...
                break;
            case 'telegram':
                shareLink = `https://telegram.me/share/url?url=${encodeURIComponent(shareUrl + `/shared/dasam-granth/shabad/${shabad}/line/${line}`)}&text=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'whatsapp':
                shareLink = `https://api.whatsapp.com/send?text=${encodeURIComponent(`${punjabi} ${translit}` + " " + shareUrl + `/shared/dasam-granth/shabad/${shabad}/line/${line}`)}`;
                break;
            case 'mail':
                shareLink = `mailto:?subject=${encodeURIComponent(`${punjabi} ${translit}`)}&body=${encodeURIComponent(shareUrl + `/shared/dasam-granth/shabad/${shabad}/line/${line}`)}`;
                break;
            default:
                break;
        }
                        ...
                        <div className='col-lg-6 p-0'>
                            <div className='in-act-wrapper'>
                                <div className='actions-mains'>
                                    ...
                                    <button className='action-btn-main' onClick={() => window.open(`/dasam-granth/shabad/${shabad_id}/line/${lineno}/print-view`, '_blank', 'height=700,width=700')}><i class="bi bi-printer"></i></button>
                            ...
                            {isSplitView === false ?
                                <div className='ang-wrapper' ref={textContainerRef}>
                                    {angData.map((item, index) => {
                                        ...
                                        console.log('COLORitem', parseInt(item.shabdlineID) === parseInt(lineno))
                                        console.log('COLORline', lineno)
                                        return (
                                                ...
                                                {isSocialShare ?
                                                    ...
                                                    <div className='socia-share' style={{ marginLeft: '600px' }}> 
                                                        <ul>
                                                            ...
                                                            <li><button className='ang-btn'  onClick={(e) => { e.preventDefault(); navigate('/dasam-granth/verse/' + `${item.verseID}`,{ state: { line: item.pagelineID }} )}}>Verse View</button></li>
                                                            <li>
                                                                <Link className='soc-icon' onClick={() => handleShareModal('facebook', item.shabad_id, item.shabadlineno, item.punjabi,item.translit)} >
                                                                    <img src={facebook} class="img-fluid donate" alt="Responsive image" />
                                                                </Link>
                                                            </li>
                                                            <li>
                                                                <Link className='soc-icon' onClick={() => handleShareModal('twitter', item.shabad_id, item.shabadlineno, item.punjabi,item.translit)} >
                                                                    <img src={twitter} class="img-fluid donate" alt="Responsive image" />
                                                                </Link>
                                                            </li>
                                                            <li>
                                                                <Link className='soc-icon' onClick={() => handleShareModal('telegram', item.shabad_id, item.shabadlineno, item.punjabi,item.translit)} >
                                                                    <img src={telegram} class="img-fluid donate" alt="Responsive image" />
                                                                </Link>
                                                            </li>
                                                            <li>
                                                                <Link className='soc-icon' onClick={() => handleShareModal('whatsapp', item.shabad_id, item.shabadlineno, item.punjabi,item.translit)} >
                                                                    <img src={whatsapp} class="img-fluid donate" alt="Responsive image" />
                                                                </Link>
                                                            </li>
                                                            <li>
                                                                <Link className='soc-icon' onClick={() => handleShareModal('mail', item.shabad_id, item.shabadlineno, item.punjabi,item.translit)} >
                                ...
                                <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                    {angData.map((item,index) =>(
                                         <div className={`lang-6 ${parseInt(lineno) === parseInt(item.shabdlineID) ? 'ang-itm-shabad' : ''}`} style={{ fontFamily: englishFont, fontSize: `${englishSize}px`, color: englishColor }}>
                                         {item.teeka}
                                       </div>
                                    ))}
                                </div> : null}
                                {isAttrib ?
                                        <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                            {angData.map((item, index) => (
                                                <>
                                                    <div className={`lang-6 ${parseInt(lineno) === parseInt(item.shabdlineID) && 'ang-itm-shabad' }`} style={{ fontFamily: attribFont, fontSize: `${attribSize}px`, color: attribColor }}>{item.attributes} </div>
                                                </>
                                            ))}
                                        </div> : null}
