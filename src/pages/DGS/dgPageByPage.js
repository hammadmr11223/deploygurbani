...
const DGPageByPage = (props) => {
    ...
    const handleShareModal = (platform,page,line,punjabi,translit) => {
        ...
        switch (platform) {
            case 'facebook':
                shareLink = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl + `/shared/dasam-granth/page/${page}/line/${line}`)}&t=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'twitter':
                shareLink = `https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl + `/shared/dasam-granth/page/${page}/line/${line}`)}&text=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'youtube':
                ...
                break;
            case 'telegram':
                shareLink = `https://telegram.me/share/url?url=${encodeURIComponent(shareUrl + `/shared/dasam-granth/page/${page}/line/${line}`)}&text=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'whatsapp':
                shareLink = `https://api.whatsapp.com/send?text=${encodeURIComponent(`${punjabi} ${translit}` + " " + shareUrl + `/shared/dasam-granth/page/${page}/line/${line}`)}`;
                break;
            case 'mail':
                shareLink = `mailto:?subject=${encodeURIComponent(`${punjabi} ${translit}`)}&body=${encodeURIComponent(shareUrl + `/shared/dasam-granth/page/${page}/line/${line}`)}`;
                break;
            default:
                break;
        }
                    ...
                    <div className='row'>
                        <div className='col-lg-12 p-0'>
                            <div className='in-act-wrapper'>
                                <h1 className='inner-heading' >Sri Dasam Granth Sahib</h1>
                                <div className='actions-mains'>
                                    ...
                                    <button className='action-btn-main' onClick={() => window.open(`/dasam-granth/page/${angNo}/print-view`, '_blank', 'height=700,width=700')}><i class="bi bi-printer"></i></button>
            ...
            <section>
                <div className='container'>
                    <div className='d-flex flex-column'>
                        ...
                        <h1>Displaying Page {headingData.current_page} of 2820</h1>
                            {isSplitView === false ?
                                <div className='ang-wrapper' ref={textContainerRef}>
                                    {angData.map((item, index) => {
                                        ...
                                        return (
                                            <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'} ${parseInt(props.lineNo) === parseInt(item.pagelineno) && 'ang-itm-shabad' }`} >
                                                ...
                                                {isSocialShare ?
                                                    ...
                                                    <ul>
                                                            <li><button className='ang-btn' onClick={(e) => { e.preventDefault(); navigate.push('/DGS/shabad/' + `${item.shabdID}` + '/line/' + `${item.shabdlineID}`) }}>Shabad View</button></li>
                                                            {/* <li><button className='ang-btn'  onClick={(e) => { e.preventDefault(); navigate.push('/DGS/verse/' + `${item.verseID}` ,{ state: { line: item.pagelineID }} )}}>Verse View</button></li> */}
                                                               <li>
                                                                <button
                                                                    className="ang-btn"
                                                                    onClick={(e) => {
                                                                        e.preventDefault();
                                                                        navigate.push(`/DGS/verse/${item.verseID}?line=${item.pagelineID}`);
                                                                    }}
                                                                >
                                                                    Verse View
                                                                </button>
                                ...
                                <div className='ang-wrapper' ref={textContainerRef}>
                                    <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                        {angData.map((item,index) =>{
                                            const characters = item.punjabi.split(' ');
                                            return(
                                                isLareevar && isLareevarAssist === false ? <div className={`${parseInt(props.lineNo) === parseInt(item.pagelineno) && 'ang-itm-shabad' }`} style={{ fontFamily: gurmukhiFont, fontSize: `${gurmukhiSize}px`, color: gurmukhiColor }}>{item.punjabi.replace(/\s+/g, '')}</div> :
                                                isLareevar && isLareevarAssist ?
                                                    characters.map((char, index) => (
                                                        <span className={`lang-1 ${parseInt(props.lineNo) === parseInt(item.pagelineno) && 'ang-itm-shabad' }`}
                                                            key={index}
                                                            style={{ color: index % 2 === 0 ? gurmukhiColor : 'red', fontFamily: gurmukhiFont, fontSize: `${gurmukhiSize}px`, }}
                                                        >{char}
                                                        </span>
                                                    )) : isPunctuation && isPunctuationAssist === false ? <div className={`${parseInt(props.lineNo) === parseInt(item.pagelineno) && 'ang-itm-shabad' }`} style={{ fontFamily: gurmukhiFont, fontSize: `${gurmukhiSize}px`, color: gurmukhiColor }} >{item.punctuation}</div> :
                                                        isPunctuation && isPunctuationAssist ?
                                                        <div style={{ fontFamily: gurmukhiFont, fontSize: `${gurmukhiSize}px`, color: gurmukhiColor }}  dangerouslySetInnerHTML={{ __html: processText(item.punctuation) }} ></div>
                                                        : isMouse ? < MouseOverDic  content={item.punjabi} keyWord={headingData?.keywords} 
                                                        mouse={isMouse} punctation={isPunctuation} lareevar={isLareevar} 
                                                        gurmukhiFont={gurmukhiFont} gurmukhiColor={gurmukhiColor} gurmukhiSize={gurmukhiSize}/> : 
                                                        isGurumukhi ?
                                                            <div className={`${parseInt(props.lineNo) === parseInt(item.pagelineno) && 'ang-itm-shabad' }`} style={{ fontFamily: gurmukhiFont, fontSize: `${gurmukhiSize}px`, color: gurmukhiColor }} >{item.punjabi}</div> : null                                       
                                    ...
                                    <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                        {angData.map((item,index) =>(
                                             <div className={`${parseInt(props.lineNo) === parseInt(item.pagelineno) && 'ang-itm-shabad' }`} style={{ fontFamily: phoneticFont, fontSize: `${phoneticSize}px`, color: phoneticColor }} >{item.translit}</div> 
                                        ))}
                                    </div> : null}
                                    {isEnglish ?
                                    <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                        {angData.map((item,index) =>(
                                             <div className={`lang-3 ${parseInt(props.lineNo) === parseInt(item.pagelineno) && 'ang-itm-shabad' }`} style={{ fontSize: `${englishTranslitSize}px`, color: englishTranslitColor }} >{item.roman}</div> 
                                        ))}
                                    </div>: null}
                                    { isHindi ?
                                    <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                        {angData.map((item,index) =>(
                                            <div className={`${parseInt(props.lineNo) === parseInt(item.pagelineno) && 'ang-itm-shabad' }`} style={{ fontFamily: hindiFont, fontSize: `${hindiSize}px`, color: hindiColor }}>{item.hindi}</div> 
                                        ))}
                                    </div> : null}
                                    {isShahmukhi ?
                                    <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                        {angData.map((item,index) =>(
                                             <div className={`lang-6 ${parseInt(props.lineNo) === parseInt(item.pagelineno) && 'ang-itm-shabad' }`} style={{ fontSize: `${shahmukhiSize}px`, color: shahmukhiColor }}>{item.urdu}</div> 
                                        ))}
                                    </div> : null}
                                    {isSantSinghTransln ?
                                    <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                        {angData.map((item,index) =>(
                                             <div className={`${parseInt(props.lineNo) === parseInt(item.pagelineno) && 'ang-itm-shabad' }`} style={{ fontFamily: englishFont, fontSize: `${englishSize}px`, color: englishColor }}>{item.english}</div> 
                                        ))}
                                    </div> : null}
                                    {isTeekaTranslnDG ?
                                    <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                        {angData.map((item,index) =>(
                                             <div className={`lang-6 ${parseInt(props.lineNo) === parseInt(item.pagelineno) && 'ang-itm-shabad' }`} style={{ fontFamily: englishFont, fontSize: `${englishSize}px`, color: englishColor }}>{item.teeka}</div> 
                                        ))}
                                    </div> : null}
                                    {isAttrib ?
                                        <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                            {angData.map((item, index) => (
                                                <>
                                                    <div className={`lang-6 ${parseInt(props.lineNo) === parseInt(item.pagelineno) && 'ang-itm-shabad' }`} style={{ fontFamily: attribFont, fontSize: `${attribSize}px`, color: attribColor }}>{item.attributes} </div>
                                                </>
                                            ))}
                                        </div> : null}
