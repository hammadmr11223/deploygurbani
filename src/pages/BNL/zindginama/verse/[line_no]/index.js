...
const BnlVerse = (props) => {
   /*  const location = useLocation();
    const { type, page_no } = useParams(); */
     const searchParams = useSearchParams();
/*   const { verseID, type } = useParams(); */
 const params = useParams();
            const type = "zindginama";
           const verseID = params?.line_no;


  // Get 'line' from query string
  const page_no = verseID //searchParams.get('line');
...
console.log("apiName", type);
    // Save values from item into localStorage
    localStorage.setItem('title', item.punjabi);
    localStorage.setItem('pageTitle', item.pageID);

  
    const url = `/BNL/${type}/shabad/${item.shabadID}/line/${item.shabadlineID}`;
    router.push(url);
  };
  
  const handleClickPage = (e, item) => {
    e.preventDefault();


    // Navigate to dynamic route
   const url = `/BNL/${type}/page/${item.pageID}/line/${item.pagelineID}`;
    router.push(url);
  };
                    ...
                    <div className='row'>
                        <div className='col-lg-12 p-0'>
                            <div className='in-act-wrapper'>
                                {type === 'ghazal' ?
                                    <h1 className='inner-heading' >Bhai Nand Lal -Divan-e-Goya: Ghazals Verse</h1> :
                                    type === 'quatrains' ?
                                        <h1 className='inner-heading' >Bhai Nand Lal - Rubaayee Verse</h1> :
                                        type === 'zindginama' ?
                                            <h1 className='inner-heading' >Bhai Nand Lal - Zindginama Verse</h1> :
                                            type === 'ganjnama' ?
                                                <h1 className='inner-heading' >Bhai Nand Lal - Ganjnama Verse</h1> :
                                                type === 'jot-bikas' ?
                                                    <h1 className='inner-heading' >Bhai Nand Lal - Jot Bikas Verse</h1> :
                                                    type === 'jot-bikas-persian' ?
                                                        <h1 className='inner-heading' >Bhai Nand Lal - Jot Bikas Verse</h1> :
                                                        type === 'rahitnama' ?
                                                            <h1 className='inner-heading' >Bhai Nand Lal - Rahitnama Verse</h1> :
                                                            type === 'tankahnama' ?
                                                                <h1 className='inner-heading' >Bhai Nand Lal - Tankahnama Verse</h1> : null}
                            ...
                            <div className='col-lg-6 p-0'>
                                <div className='in-act-wrapper'>
                                    <div className='actions-mains'>
                                        ...
                                        <button className='action-btn-main' onClick={() => window.open(`/BNL/${type}/verse/${page_no}/print-view`, '_blank', 'height=700,width=700')}><i class="bi bi-printer"></i></button>
                                            ...
                                            /> */}{/* } */}
                                                ...
                                                {isSocialShare ?
                                                    ...
                                                    <div className='socia-share' style={{ marginLeft: '600px' }}>
                                                        <ul>
                                                           {/*  <li><button className='ang-btn' onClick={(e) => { e.preventDefault(); navigate(`/bhai-nand-lal/${type}/page/${item.pageID}/line/${item.pagelineID}`) }}>Page View</button></li>
                                                            <li><button className='ang-btn' onClick={(e) => { e.preventDefault(); navigate(`/bhai-nand-lal/${type}/shabad/${item.shabadID}/line/${item.shabadlineID}`, { state: { Title: item.punjabi, pageTitle: item.pageID } }) }}>Shabad View</button></li> */}
