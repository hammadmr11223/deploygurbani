...
import { formatTextForCopyBnl } from '@/components/TextCopyBnl';
...
/* const { type, shabad_id, lineno } = useParams(); */

                const params = useParams();
            const type = "rahitnama";
            const shabad_id = params?.shabad_id;
  const lineno = params?.line_no;
console.log("object", shabad_id, "lineno",lineno );
...
pageTitle
  ...
  console.log("daa", data);
    ...
    const getShabadLines = async (id) => {
        ...
        await ApiHelper.get(API.getBnlPages + type + '/shabad/' + id + "/line/" + lineno + '/title/' +  TitleNew)
            .then((resData) => {
                setLoader(false);
    ...
    const handleShareModal = (platform,shabad,line,punjabi,translit) => {
        ...
        switch (platform) {
            case 'facebook':
                shareLink = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl + `/shared/bhai-nand-lal/${type}/shabad/${shabad}/line/${line}`)}&t=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'twitter':
                shareLink = `https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl + `/shared/bhai-nand-lal/${type}/shabad/${shabad}/line/${line}`)}&text=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'youtube':
                ...
                break;
            case 'telegram':
                shareLink = `https://telegram.me/share/url?url=${encodeURIComponent(shareUrl + `/shared/bhai-nand-lal/${type}/shabad/${shabad}/line/${line}`)}&text=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'whatsapp':
                shareLink = `https://api.whatsapp.com/send?text=${encodeURIComponent(`${punjabi} ${translit}` + " " + shareUrl + `/shared/bhai-nand-lal/${type}/shabad/${shabad}/line/${line}`)}`;
                break;
            case 'mail':
                shareLink = `mailto:?subject=${encodeURIComponent(`${punjabi} ${translit}`)}&body=${encodeURIComponent(shareUrl + `/shared/bhai-nand-lal/${type}/shabad/${shabad}/line/${line}`)}`;
                break;
            default:
                break;
        }
   ...
   const url = `/BNL/${type}/page/${item.pageID}/line/${item.lineno}`;
    router.push(url);
  };

      const handleClickVerse = (e, item) => {
    e.preventDefault();


    // Navigate to dynamic route
   const url = `/BNL/${type}/verse/${item.verseID}?line=${item.lineno}`;
    router.push(url);
  };
    ...
    return (
        <div>
            ...
            <section className='section-shabad p-5'>
                <div className='container'>
                    ...
                    {/* <h3 className='text-dark mb-3 text-center' >{data?.Title}</h3> */}
                    <h3 className='text-dark mb-3 text-center' >ਹਵਾਇ ਬੰਦਗੀ ਆਵੁਰਦ ਦਰ ਵਜੂਦ ਮਰਾ</h3>
                   {/*  <p className='text-dark mb-2 text-center' ><b>{`This shabad is on page   ${data?.pageTitle}   of Bhai Nand Lal.`}</b></p>  */}
                    <p className='text-dark mb-2 text-center' ><b>{`This shabad is on page   ${lineno}   of Bhai Nand Lal.`}</b></p> 
                </div>
            </section>
                        ...
                        <div className='col-lg-6 p-0'>
                            <div className='in-act-wrapper'>
                                <div className='actions-mains'>
                                    ...
                                    <button className='action-btn-main' onClick={() => window.open(`/BNL/${type}/shabad/${shabad_id}/line/${lineno}/${headingData?.title}/print-view`, '_blank', 'height=700,width=700')}><i className="bi bi-printer"></i></button>
                                            ...
                                            />} */}
                                                ...
                                                {isSocialShare ?
                                                    ...
                                                    <div className='socia-share' style={{ marginLeft: '600px' }}>
                                                        <ul>
                                                          {/*   <li><button className='ang-btn' onClick={(e) => { e.preventDefault(); navigate(`/bhai-nand-lal/${type}/page/${item.pageID}/line/${item.lineno}`) }}>Page View</button></li>
                                                            <li><button className='ang-btn' onClick={(e) => { e.preventDefault(); navigate(`/bhai-nand-lal/${type}/verse/${item.verseID}`, { state: { line: item.lineno }}) }}>Verse View</button></li>
                                                            */} 
      ...
      >
        Page View
                                                      ...
                                                      <li> <button
        className="ang-btn"
        onClick={(e) => handleClickVerse(e, item)} 
      >
        Verse View
      </button></li>
                                ...
                                <div className='ang-wrapper'>
                                    <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                        {angData.map((item, index) => {
                                            const characters = item.punjabi.split(' ');
                                            return (
                                                isLareevar && isLareevarAssist === false ? <div className={`${parseInt(lineno) === parseInt(item.shabadlineno) ? 'ang-itm-shabad' : ''}`} style={{ fontFamily: gurmukhiFont, fontSize: `${gurmukhiSize}px`, color: gurmukhiColor }}>{item.punjabi.replace(/\s+/g, '')}</div> :
                                                    isLareevar && isLareevarAssist ?
                                                        characters.map((char, index) => (
                                                        ...
                                                        )) : isPunctuation && isPunctuationAssist === false ? <div className={`${parseInt(lineno) === parseInt(item.shabadlineno) ? 'ang-itm-shabad' : ''}`} style={{ fontFamily: gurmukhiFont, fontSize: `${gurmukhiSize}px`, color: gurmukhiColor }} >{item.punctuation}</div> :
                                                            isPunctuation && isPunctuationAssist ?
                                                                characters.map((char, index) => (
                                                                    <span className={`lang-1 ${parseInt(lineno) === parseInt(item.shabadlineno) ? 'ang-itm-shabad' : ''}`}
                                                                        key={index}
                                                                    ...
                                                                    gurmukhiFont={gurmukhiFont} gurmukhiColor={gurmukhiColor} gurmukhiSize={gurmukhiSize} /> :
                                                                    isGurumukhi ?
                                                                        <div className={`${parseInt(lineno) === parseInt(item.shabadlineno) ? 'ang-itm-shabad' : ''}`} style={{ fontFamily: gurmukhiFont, fontSize: `${gurmukhiSize}px`, color: gurmukhiColor }} >{item.punjabi}</div> : null
                                    ...
                                    {isTeekaTranslnBNL ?
                                        <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                            {angData.map((item, index) => (
                                                <div className={`lang-6 ${parseInt(lineno) === parseInt(item.shabadlineno) ? 'ang-itm-shabad' : ''}`} style={{ fontFamily: englishFont, fontSize: `${englishSize}px`, color: englishColor }}>{item.teeka}</div>
                                            ))}
                                        </div> : null}
                                    {isTeekaHindiTranslnBNL ?
                                        <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                            {angData.map((item, index) => (
                                                <div className={`lang-6 ${parseInt(lineno) === parseInt(item.shabadlineno) ? 'ang-itm-shabad' : ''}`} style={{ fontFamily: englishFont, fontSize: `${englishSize}px`, color: englishColor }}>{item.teekahindi}</div>
                                            ))}
                                        </div> : null}
                                    {isAttrib ?
                                        <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                            {angData.map((item, index) => (
                                                <>
                                                    <div className={`${parseInt(lineno) === parseInt(item.shabadlineno) ? 'ang-itm-shabad' : ''}`} style={{ fontFamily: attribFont, fontSize: `${attribSize}px`, color: attribColor }}>
                                                        {item.attributes} </div>
                                                </>
                                            ))}
                                        </div> : null}
