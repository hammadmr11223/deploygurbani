...
const KabitByKabit = (props) => {
    ...
    useEffect(() => {
        ...
        if (savedPreference) {
            ...
            if (preferences.translation.ks.teeka === true) {
                selectedTrans.push({ label: "Teeka", value: "Teeka" })
            }
            if (preferences.translation.ks.teeka_roman === true) {
                selectedTrans.push({ label: "Teeka Roman", value: "Teeka Roman" })
            }
            if (preferences.translation.ks.teeka_hindi === true) {
    ...
    const handleSelectTranslan = (event) => {
        ...
        if (selectedOption === "English") {
            ...
            setIsTeekaTranslnKK(!isTeekaTranslnKK)
            setPref((prevPref) => {
                const updatedPref = {
                    ...prevPref, translation: {
                        ...prevPref.translation,
                        ks: { ...prevPref.translation.ks, teeka: !isTeekaTranslnKK }
                    }
                };
                localStorage.setItem('Preference', JSON.stringify(updatedPref));
                return updatedPref;
            ...
            setIsTeekaRomanTranslnKK(!isTeekaRomanTranslnKK)
            setPref((prevPref) => {
                const updatedPref = {
                    ...prevPref, translation: {
                        ...prevPref.translation,
                        ks: { ...prevPref.translation.ks, teeka_roman: !isTeekaRomanTranslnKK }
                    }
                };
                localStorage.setItem('Preference', JSON.stringify(updatedPref));
                return updatedPref;
            ...
            setIsTeekaHindiTranslnKK(!isTeekaHindiTranslnKK)
            setPref((prevPref) => {
                const updatedPref = {
                    ...prevPref, translation: {
                        ...prevPref.translation,
                        ks: { ...prevPref.translation.ks, teeka_hindi: !isTeekaHindiTranslnKK }
                    }
                };
                localStorage.setItem('Preference', JSON.stringify(updatedPref));
                return updatedPref;
        ...
        const handleSelectTranslan = (selectedList) => {
            ...
            setIsTeekaTranslnKK(isTeekaTranslnSelected);
            setIsTeekaRomanTranslnKK(isTeekaRomanTranslnSelected);
            setIsTeekaHindiTranslnKK(isTeekaHindiTranslnSelected);
    ...
    const handleReset = () => {
        ...
        setIsTeekaTranslnKK(false);
        setIsTeekaRomanTranslnKK(false);
        setIsTeekaHindiTranslnKK(false);
        setIsCenter(false);
        setIsLareevar(false);
        setIsLareevarAssist(false);
        setIsPunctuationAssist(false);
    ...
    const handleShareModal = (platform,page,line,punjabi,translit) => {
        ...
        switch (platform) {
            case 'facebook':
                shareLink = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl + `/shared/kabit-savaiye/kabit/${page}/line/${line}`)}&t=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'twitter':
                shareLink = `https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl + `/shared/kabit-savaiye/kabit/${page}/line/${line}`)}&text=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'youtube':
                ...
                break;
            case 'telegram':
                shareLink = `https://telegram.me/share/url?url=${encodeURIComponent(shareUrl + `/shared/kabit-savaiye/kabit/${page}/line/${line}`)}&text=${encodeURIComponent(`${punjabi} ${translit}`)}`;
                break;
            case 'whatsapp':
                shareLink = `https://api.whatsapp.com/send?text=${encodeURIComponent(`${punjabi} ${translit}` + " " + shareUrl + `/shared/kabit-savaiye/kabit/${page}/line/${line}`)}`
                break;
            case 'mail':
                shareLink = `mailto:?subject=${encodeURIComponent(`${punjabi} ${translit}`)}&body=${encodeURIComponent(shareUrl + `/shared/kabit-savaiye/kabit/${page}/line/${line}`)}`;
                break;
            default:
                break;
        }
    ...
    const handleCopyText = () => {
        if (textContainerRef.current) {
            const textToCopy = formatTextForCopyBvgAndKs(angData, isLareevar, isLareevarAssist, isPunctuation, isPunctuationAssist,
                isPhonetic, isGurumukhi, isHindi, isEnglish, isShahmukhi, isSantSinghTransln, isTeekaTranslnKK, isTeekaRomanTranslnKK, isTeekaHindiTranslnKK, isAttrib
            );
                    ...
                    <div className='row'>
                        <div className='col-lg-12 p-0'>
                            <div className='in-act-wrapper'>
                                <h1 className='inner-heading' >Kabit Savaiye</h1>
                                <div className='actions-mains'>
                                    ...
                                    <button className='action-btn-main' onClick={() => window.open(`/kabit-savaiye/kabit/${angNo}/print-view`, '_blank', 'height=700,width=700')}><i class="bi bi-printer"></i></button>
                ...
                <div className='container'>
                    <div className='d-flex flex-column'>
                        <div className='ang-display mt-5'>
                            <h1>Displaying Kabit {headingData.current_page} of 675</h1>
                            {isSplitView === false ?
                                    ...
                                    {angData.map((item, index) => {
                                        ...
                                        return (
                                            <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                                ...
                                                {isTeekaTranslnKK ? <div className='mt-4' style={{ fontFamily: englishFont, fontSize: `${englishSize}px`, color: englishColor }}>{item.teeka_punjabi}</div> : null}
                                                {isTeekaRomanTranslnKK ? <div className='mt-4' style={{ fontFamily: englishFont, fontSize: `${englishSize}px`, color: englishColor }} >{item.teeka_roman}</div> : null}
                                                {isTeekaHindiTranslnKK ? <div className='mt-4 mb-4' style={{ fontFamily: englishFont, fontSize: `${englishSize}px`, color: englishColor }} >{item.teeka_hindi}</div> : null}
                                                {isAttrib ? <div style={{ fontFamily: attribFont, fontSize: `${attribSize}px`, color: attribColor }}>{item.lattrib} </div> : null}
                                                {isSocialShare ?

                                                    <div className='socia-share' style={{ marginLeft: '600px' }}>
                                                        <ul>
                                                            <li>
                                                                <a className='soc-icon' onClick={() => handleShareModal('facebook' , item.kabit, item.lineno, item.punjabi,item.translit)} >
                                                                    <Image src={facebook} class="img-fluid donate" alt="Responsive image" />
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a className='soc-icon' onClick={() => handleShareModal('twitter', item.kabit, item.lineno, item.punjabi,item.translit)} >
                                                                    <Image src={twitter} class="img-fluid donate" alt="Responsive image" />
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a className='soc-icon' onClick={() => handleShareModal('telegram', item.kabit, item.lineno, item.punjabi,item.translit)} >
                                                                    <Image src={telegram} class="img-fluid donate" alt="Responsive image" />
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a className='soc-icon' onClick={() => handleShareModal('whatsapp', item.kabit, item.lineno, item.punjabi,item.translit)} >
                                                                    <Image src={whatsapp} class="img-fluid donate" alt="Responsive image" />
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a className='soc-icon' onClick={() => handleShareModal('mail', item.kabit, item.lineno, item.punjabi,item.translit)} >
                                ...
                                <div className='ang-wrapper' ref={textContainerRef}>
                                    ...
                                    {isTeekaTranslnKK ?
                                        <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                            {angData.map((item, index) => (
                                                <div style={{ fontFamily: englishFont, fontSize: `${englishSize}px`, color: englishColor }} >{item.teeka_punjabi}</div>
                                            ))}
                                        </div> : null}
                                    {isTeekaRomanTranslnKK ?
                                        <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                            {angData.map((item, index) => (
                                                <div style={{ fontFamily: englishFont, fontSize: `${englishSize}px`, color: englishColor }} >{item.teeka_roman}</div>
                                            ))}
                                        </div> : null}
                                    {isTeekaHindiTranslnKK ?
                                        <div className={`ang-itm ${isCenter && 'center-align'} ${isDarkMode && 'dark-mode'}`} >
                                            {angData.map((item, index) => (
